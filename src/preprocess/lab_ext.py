...
def ellipse_to_bbox(x, y, radiusX, radiusY, image_width, image_height):
    ...
    x_max = min(image_width, (x + radiusX) * image_width / 100)
    y_min = max(0, (y - radiusY) * image_height / 100)
    y_max = min(image_height, (y + radiusY) * image_height / 100)
...
class LoadData:
    ...
    def extract_labels():
        ...
        f = open(str(PROJECT_ROOT) + "\\dataset\\labels\\labels.json")
        data = json.load(f)
        for i in range(11000):
            if 'resized' in data[i]['data']['image']:
                ...
                reg_idx = torch.tensor([idx, idx, idx, idx])
                ...
                try:
                    ...
                    if 'Z' in choices:
                        zero_tensor, cls_zero_tensor, zero_holes_tensor = torch.zeros([26, 4, 1]), torch.zeros([26, 5, 1]), torch.zeros([26,1,1])
                        ...
                        if regression_tensor is None:
                            ...
                            regression_tensor = torch.zeros([26, 4, 1])
                        ...
                        reg_tensor_to_fill = torch.zeros([26, 4])
                        ...
                        for previous, item, next in LoadData.previous_and_next(data[i]['annotations'][0]['result']):
                            if next is None:
                                ...
                                if "U" in choices:
                                    ...
                                    x_t, x_b = max(0,item['value']['x']-item['value']['radiusX']), min(384,item['value']['y']+item['value']['radiusY'])
                                    y_t, y_b = max(0,item['value']['y']-item['value']['radiusY']), min(275,item['value']['x']+item['value']['radiusX'])
                                    to_add = torch.tensor([x_t,y_t,x_b,y_b])
                                ...
                                if "U" in choices:
                                    ...
                                    cls_to_add = torch.tensor(cls_to_add)
                                    cls_tensor_to_fill[count, :] = cls_to_add
                                    x_t, y_t = max(0,item['value']['x']-item['value']['radiusX']), max(0,item['value']['x']-item['value']['radiusY'])
                                    x_b, y_b = min(275,item['value']['x']+item['value']['radiusX']), min(275,item['value']['x']+item['value']['radiusY'])
                                    reg_to_add = torch.tensor([x_t, y_t, x_b, x_b])
    ...
    def extract_images(directory_in_str):
        ...
        torch.save(dir_of_images, str(PROJECT_ROOT) + "\\dataset\\torched\\torched_images")
